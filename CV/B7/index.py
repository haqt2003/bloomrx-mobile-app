...
cv2.imwrite('blue_channel.jpg', Blue_channel)  # Lưu ảnh kênh xanh dương

# Kênh xanh lá (Green)
Green_channel = cv2.merge([zero_channel, G, zero_channel])
cv2.imshow('Green Channel (Green)', Green_channel)
cv2.imwrite('green_channel.jpg', Green_channel)  # Lưu ảnh kênh xanh lá

# Kênh đỏ (Red)
Red_channel = cv2.merge([zero_channel, zero_channel, R])
cv2.imshow('Red Channel (Red)', Red_channel)
cv2.imwrite('red_channel.jpg', Red_channel)  # Lưu ảnh kênh đỏ
...
cv2.imwrite('modified_image.jpg', image_modified)  # Lưu ảnh đã chỉnh sửa
